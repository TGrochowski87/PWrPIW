...
const searchInLists = (event) => {
  ...
  if (checkbox.checked) {
    ...
    for (let i = 0; i < listOdLists.children.length; i++) {
      ...
      for (let j = 2; j < list.children.length; j++) {
        if (
          list.children[j].children[0].textContent
            .toLowerCase()
            .includes(toSearchFor.toLowerCase())
        ) {
          list.children[j].style.display = "block";
        } else {
          list.children[j].style.display = "none";
  ...
  for (let i = 2; i < items.length; i++) {
    if (items[i].style.display === "none") {
      if (checkbox.checked) {
        ...
        if (
          searchText === "" ||
          items[i].textContent.toLowerCase().includes(searchText.toLowerCase())
        ) {
          items[i].style.display = "block";
        }
      }
...
const addToDoItem = (event) => {
  ...
  newText.onclick = (event) => {
    setItemStatus(event.target, newDate);
  };
